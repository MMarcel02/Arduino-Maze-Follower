...
void bangLineFollowEmergencyStop() {

  switch (bangBangLineFollowState) {
  case (BangBangLineFollowing):
    if (obstacleFound()) {
      stopAllMotors();
      bangBangLineFollowState = ObstacleInFront;
      break;
    }
    // If doesn't see black at all we assume we are following the line so we go
    // forward
    ...
    // Detect on both so lets use last direction we were headed in (probably on
    // a 90 degree turn)
